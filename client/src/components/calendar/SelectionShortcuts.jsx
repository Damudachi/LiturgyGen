...
const chip =
  'lift inline-flex min-h-10 cursor-pointer items-center rounded-full px-4 text-sm font-semibold ring-1 ring-inset disabled:cursor-not-allowed disabled:opacity-50';
    ...
    <div className="animate-panel flex flex-wrap items-center gap-2" role="group" aria-label="Add days quickly">
      ...
      {SHORTCUTS.map((shortcut) => (
        <button
          ...
          className={cx(chip, 'bg-tile text-navy shadow-rest ring-tile-edge hover:bg-tile-sun')}
        >
          {shortcut.label}
        </button>
      ))}
      <button type="button" disabled={busy} onClick={onScheduled} className={cx(chip, 'bg-tile text-navy shadow-rest ring-tile-edge hover:bg-tile-sun')}>
        Scheduled Masses
      </button>
      <button
        type="button"
